 ...
 * main.c
 */
unsigned int resultado, ppm, ADC, RL, ppmf;
float  A, B, J5, J0;
...
int main(void)
{
    WDTCTL = WDTPW | WDTHOLD;   // stop watchdog timer
       ...
       while(1){
           ...
           resultado = ADC10MEM;
           J5 = (1024*(RL/resultado))- RL;
           J0 = (J5/(A * (pow(ppm,B))));
           ppm = ((1024*(RL/resultado)-RL)/(J0*A));
           ppmf = pow(ppm, 1/B);
           centenas = ((ppmf / 100) + 0x30);
           decenas = (((ppmf%100)/10) + 0x30);
           unidades = ((ppmf%10) + 0x030);
